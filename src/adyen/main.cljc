(ns adyen.main
  "Kotodama WASM entrypoint for the Adyen clean-room actor (L4) — Clojure port.
  ...
  facts (`adyen.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Payment"           :plural "payments"           :id-prefix "adyen_pay"
    :fields [:pspReference :merchantReference :merchantAccount :reference :resultCode :shopperReference]
    :required [:pspReference :merchantReference]
    :coerce {}                       :refs {}}
   {:entity "Amount"            :plural "amounts"            :id-prefix "adyen_amo"
    :fields [:value :currency]                               :required [:value :currency]
    :coerce {:value :int}            :refs {}}
   {:entity "ModificationResult" :plural "modificationresults" :id-prefix "adyen_mod"
    :fields [:pspReference :resultCode :response]            :required [:pspReference :resultCode]
    :coerce {}                       :refs {}}
   {:entity "PaymentMethod"     :plural "paymentmethods"     :id-prefix "adyen_mth"
    :fields [:type :storedPaymentMethodId]                   :required [:type]
    :coerce {}                       :refs {}}
   {:entity "Notification"      :plural "notifications"      :id-prefix "adyen_not"
    :fields [:eventCode :pspReference :merchantReference :eventDate :success]
    :required [:eventCode :pspReference]
    :coerce {:success :bool}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"adyen.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "adyen-compat" :tier tier :entities entities} 200])
